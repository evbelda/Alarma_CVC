...
void TelegramCom::TelegramServidor(RelojAlarma *reloj, AlarmaC *alcObj, LectorSD *objSD, RTC *rtc, UniversalTelegramBot *bot) {
	...
	String mensaje;
	int CHora = 0; //valor del mensaje de cambio de hora
	//Serial.println("Estado: " + String(estado) + "-Chat_id: " + chat_id + " numMensajes: " + String(numNewMessages));
	char *p,*i;//para convertir el PIN y saber si tiene 4 dijitos
	if (tiempoTel < millis()) {
		...
		while (numNewMessages) {
			for (int i = 0; i<numNewMessages; i++) {
				...
				if (String(item) == "Cambio")
				{
					item = strtok(NULL, " ");
					if (String(item) == "Hora") {
						//item = strtok(NULL, " ");
						CHora = reloj->CambiaHora(rtc, item);
						if (CHora == 0)
						{
							bot->sendMessage(bot->messages[i].chat_id, "Hora Cambiada.", "");
						}
						else if (CHora == 1)
						{
							bot->sendMessage(bot->messages[i].chat_id, "Formato numeros incorrectos.", "");
						}
						else if (CHora == 2)
						{
							bot->sendMessage(bot->messages[i].chat_id, "Formato Hora incorrecto.", "");
						}
						else
						{
							bot->sendMessage(bot->messages[i].chat_id, "Formato Fecha incorrecto.", "");
						}
						//rtc->setDateTime(2018, 04, 12, 18, 06, 00);
					}
					if (String(item) == "PIN") {
						item = strtok(NULL, " ");
						if (String(item) == alcObj->secretCode)
						{
							item = strtok(NULL, " ");
							if ((strlen(item) == 4) || (isDigit(p[0])) || (isDigit(p[1])) || (isDigit(p[2])) || (isDigit(p[3])))
							{
								alcObj->secretCode = String(item);
								bot->sendMessage(bot->messages[i].chat_id, "Pin Cambiado.", "");
							}
							else
							{
								bot->sendMessage(bot->messages[i].chat_id, "El pin debe tener cuatro numeros.", "");
							}
						}
						else
						{
							bot->sendMessage(bot->messages[i].chat_id, "Pin incorrecto.", "");
						}
						//rtc->setDateTime(2018, 04, 12, 18, 06, 00);
					}
				}
